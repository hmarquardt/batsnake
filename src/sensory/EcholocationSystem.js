...
export class EcholocationSystem {
  ...
  constructor(scene,world,events,settings,pulseResources){
    ...
    this.returnPool=new EchoReturnPool(scene,24);this.pulseResources=pulseResources;this.lastUpdateMs=0;
  ...
  emit(origin,kind='full'){
    ...
    this.pulse=new EcholocationPulse(this.scene,origin,range,duration,this.pulseResources);this.maxCooldown=(quick ? .68 : 1.65)*(this.settings.get('difficulty')==='flightline'?1.12:this.settings.get('difficulty')==='field' ? .9 : 1);this.cooldown=this.maxCooldown;
