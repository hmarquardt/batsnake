...
export class SnakeRig {
  ...
  update(dt,headPosition,direction,state,charge){
    ...
    p[3].copy(this.anchor).lerp(headPosition,.48).add(this.offset.set(side*(.72+tension*1.05)+sway*.4,-.35,Math.sin(time*.7)*.16*breathing-tension*.3));p[4].copy(headPosition).addScaledVector(direction,-(1.35+tension*.55)).add(this.offset.set(-side*tension*.5,.08+tension*.05,0));p[5].copy(headPosition);
