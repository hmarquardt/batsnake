...
export class SnakeRig {
  ...
  constructor(scene,anchor,index){
    ...
    this.head=new THREE.Group();this.group.add(this.head);const neck=new THREE.Mesh(new THREE.CapsuleGeometry(.31,.5,4,10),this.scaleMaterial);neck.rotation.x=Math.PI/2;neck.position.set(0,0,-.48);neck.scale.set(1.12,.88,1);neck.castShadow=true;this.head.add(neck);
    ...
    const rostral=new THREE.Mesh(new THREE.BoxGeometry(.38,.09,.34,2,1,2),snoutMaterial);rostral.position.set(0,.055,.62);rostral.rotation.x=-.08;this.head.add(rostral);
    ...
    const mandibleGeometry=new THREE.CapsuleGeometry(.045,.38,3,7);for(const side of [-1,1]){const mandible=new THREE.Mesh(mandibleGeometry,snoutMaterial);mandible.rotation.x=Math.PI/2;mandible.position.set(side*.13,-.1,.28);this.jawPivot.add(mandible);}
    ...
    const supportOffsets=[new THREE.Vector3(-.4,1.1,-.5),new THREE.Vector3(.7,.9,.1),new THREE.Vector3(-.6,1,.5)];this.coils=new THREE.Group();this.coils.position.copy(anchor).addScaledVector(supportOffsets[index]||supportOffsets[0],.48);this.group.add(this.coils);this.coilLoops=[];
    for(let c=0;c<4;c++){const loop=new THREE.Mesh(new THREE.TorusGeometry(.7-c*.075,.16-c*.014,9,28),this.scaleMaterial);loop.rotation.x=Math.PI*.5+(c-1.5)*.11;loop.rotation.z=c*.82+this.index*.7;loop.position.set((c%2?1:-1)*.12,.03+c*.105,(c-1.5)*.09);loop.scale.set(1.28-c*.045,.78+c*.035,1);loop.userData.baseScale=loop.scale.clone();loop.userData.baseY=loop.position.y;loop.castShadow=true;loop.receiveShadow=true;this.coils.add(loop);this.coilLoops.push(loop);}
  ...
  createCaptureVisual(){const group=new THREE.Group();group.position.set(0,-.13,.6);group.rotation.x=-.18;const body=new THREE.Mesh(new THREE.CapsuleGeometry(.1,.28,3,7),new THREE.MeshStandardMaterial({color:0x241a14,roughness:.9}));body.rotation.x=Math.PI/2;group.add(body);for(const side of [-1,1]){const wing=new THREE.Mesh(wingGeometry(side),new THREE.MeshStandardMaterial({color:0x3a261c,transparent:true,opacity:.82,side:THREE.DoubleSide,depthWrite:true}));wing.scale.setScalar(1.18);group.add(wing);}group.visible=false;return group;}
  ...
  update(dt,headPosition,direction,state,charge){
    ...
    const breathing=(state==='prepare'?1-charge*.85:1)*(state==='idle'?1:.4);for(let i=0;i<this.coilLoops.length;i++){const loop=this.coilLoops[i],base=loop.userData.baseScale,compression=1-tension*(.075+i*.008),reaction=this.captureTime>0?Math.sin(time*22+i)*.035*this.captureTime:0;loop.scale.set(base.x*compression,base.y*(1+tension*.11),base.z*compression);loop.position.y=loop.userData.baseY-tension*i*.018+reaction;loop.rotation.z+=Math.sin(time*.7+i)*dt*.012*(1-tension);}
