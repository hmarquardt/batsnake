...
export class SnakeStrike {
  ...
  update(dt){this.previous.copy(this.position);if(this.state==='lunge'){this.time+=dt;const t=Math.min(1,this.time/.18);const eased=t<.3 ? .34*Math.pow(t/.3,2) : .34+(t-.3)/.7*.66;this.position.lerpVectors(this.start,this.end,eased);if(t>=1){this.state='recover';this.time=0;this.cooldown=1.25+this.charge*.65;}}else if(this.state==='capture'){this.time+=dt;if(this.time>.7){this.state='recover';this.time=0;this.cooldown=.9;}}else if(this.state==='recover'){this.time+=dt;const t=Math.min(1,this.time/this.cooldown);const recoil=t*t*(3-2*t);this.position.lerpVectors(this.end,this.start,recoil);if(t>=1){this.state='idle';this.time=0;this.charge=0;this.position.copy(this.start);}}return this.state;}
