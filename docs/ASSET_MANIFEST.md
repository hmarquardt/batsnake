...
|---|---|---|---|---|---|---|
| Optional future Cuban boa GLB (not selected for Milestone 7) | Realistic adult *Chilabothrus angulifer*; only pursue if it materially exceeds the production procedural rig in close facial/contact review | 2.5–4 m body; GLB; Y-up, +Z head-forward; root at ceiling anchor, head child addressable | Must remain spline-guidable; animation cannot own strike gameplay | Texture/LOD budget must be measured before adoption | Current project-owned `SnakeRig` is the selected production path; see `BOA_ASSET_DECISION.md` | No candidate selected, downloaded, or licensed |
...
|---|---|---|---|
| Boa visual | Original deterministic Three.js geometry and 128² generated scale-relief map in `SnakeRig.js`; MIT | Per animal: one 560-vertex / 1,092-triangle body geometry shared by normal, heat, and echo meshes; bounded tapered neck, rostral plane, paired mandibles, eyes, ten heat pits, tongue, four support-wrapped coils; no bones or mixer | No file load exists; the procedural animal is the selected production/fallback path and preserves `SnakeController` / `SnakeStrike` interfaces |
...
## Milestone 6 authored environment

| Asset | Creation / runtime form | Budget and contract | Provenance / fallback |
|---|---|---|---|
| Authored cave and hero formations | Original Three.js `BufferGeometry`, primitives, deterministic displacement, and instancing in `CaveGenerator.js` | One 144×32 variable shell; named roost, shelf, fang, split-column, drapery, fallen-pillar, gallery, and mouth compositions; 41 simplified solid obstacle proxies after clearance refinement | Project-created MIT source; no external file or runtime request. Existing routes and anchors are the fallback spatial contract. |
| Cave regional materials | Original generated limestone, roughness, seep, bedding, fracture, guano, and leaf pixels in `CaveMaterials.js` | Bounded 128² / 64² `DataTexture`s, shared materials, no downloaded texture bytes or obvious per-object allocation | Project-created MIT source; standard WebGL startup failure handles shader/material failure. |
| Roost and ecology | Original 96-instance hanging-bat silhouette mesh, instanced guano beds, sparse insects, pooled droplets/splashes | No hero rigs or mixers; colony draw count remains fixed while visible instance scale follows existing departure progress | Project-created MIT source; no zoological scan, texture, or recording. |
| Bounded exterior glimpse | Original low-poly terrain, tree/canopy instances, leaf geometry, local sky shader, moon, and haze | Scenery only beyond Moon Gate; no traversal, simulation, level, or external HDR | Project-created MIT source; cave gameplay remains complete if distant detail is reduced by profile. |
| Regional cave ambience | Original Web Audio oscillator/noise cues in `CaveAmbience.js` / `AudioManager.js` | Camera-region weighting for drips, colony wings, entrance air/insects, and deep resonance; no music or audio file | Project-created MIT source; Web Audio denial continues silently. |
